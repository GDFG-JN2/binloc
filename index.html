...
<!-- Tooltip hover untuk Peta 3D -->
<div id="peta-3d-tooltip" style="display:none;position:fixed;z-index:9998;background:#111827f0;color:#fff;padding:8px 12px;border-radius:8px;font-size:11px;line-height:1.6;max-width:180px;pointer-events:none;box-shadow:0 6px 18px #00000040;"></div>

<!-- ═══════════════════════════════
     SCREEN: AUTH (Daftar / Login)
═══════════════════════════════ -->
<div class="screen" id="screen-auth" style="flex-direction:column;justify-content:center;padding:24px;">
  <div style="text-align:center;margin-bottom:28px;">
    <div style="font-size:26px;font-weight:900;letter-spacing:1px;color:var(--text);">BINLOC</div>
    <div style="font-size:12px;color:var(--muted);margin-top:2px;">Warehouse Movement</div>
  </div>

  <div style="display:flex;gap:8px;margin-bottom:20px;background:var(--surface2);padding:4px;border-radius:12px;">
    <button id="auth-tab-login" onclick="authSwitchTab('login')" class="active" style="flex:1;padding:10px;border:none;border-radius:9px;background:var(--surface);color:var(--text);font-weight:800;font-size:13px;cursor:pointer;">Masuk</button>
    <button id="auth-tab-daftar" onclick="authSwitchTab('daftar')" style="flex:1;padding:10px;border:none;border-radius:9px;background:none;color:var(--muted);font-weight:800;font-size:13px;cursor:pointer;">Daftar</button>
  </div>

  <div id="auth-form-login">
    <label style="font-size:11px;color:var(--muted);font-weight:700;text-transform:uppercase;">Username</label>
    <input type="text" id="auth-login-username" placeholder="Username" style="width:100%;margin-top:6px;margin-bottom:12px;padding:12px 14px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:14px;box-sizing:border-box;">
    <label style="font-size:11px;color:var(--muted);font-weight:700;text-transform:uppercase;">Password</label>
    <input type="password" id="auth-login-password" placeholder="Password" style="width:100%;margin-top:6px;margin-bottom:6px;padding:12px 14px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:14px;box-sizing:border-box;">
    <div id="auth-login-error" style="display:none;color:#dc2626;font-size:12px;font-weight:700;margin-bottom:10px;"></div>
    <button id="auth-login-btn" onclick="authDoLogin()" style="width:100%;margin-top:8px;padding:14px;border-radius:12px;border:none;background:#9333ea;color:#fff;font-size:14px;font-weight:800;cursor:pointer;">Masuk</button>
  </div>

  <div id="auth-form-daftar" style="display:none;">
    <label style="font-size:11px;color:var(--muted);font-weight:700;text-transform:uppercase;">Nama Lengkap</label>
    <input type="text" id="auth-daftar-nama" placeholder="Nama lengkap" style="width:100%;margin-top:6px;margin-bottom:12px;padding:12px 14px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:14px;box-sizing:border-box;">
    <label style="font-size:11px;color:var(--muted);font-weight:700;text-transform:uppercase;">Username</label>
    <input type="text" id="auth-daftar-username" placeholder="Buat username" style="width:100%;margin-top:6px;margin-bottom:12px;padding:12px 14px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:14px;box-sizing:border-box;">
    <label style="font-size:11px;color:var(--muted);font-weight:700;text-transform:uppercase;">Password</label>
    <input type="password" id="auth-daftar-password" placeholder="Buat password (min 4 karakter)" style="width:100%;margin-top:6px;margin-bottom:6px;padding:12px 14px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:14px;box-sizing:border-box;">
    <div id="auth-daftar-error" style="display:none;color:#dc2626;font-size:12px;font-weight:700;margin-bottom:10px;"></div>
    <div id="auth-daftar-success" style="display:none;color:#16a34a;font-size:12px;font-weight:700;margin-bottom:10px;"></div>
    <button id="auth-daftar-btn" onclick="authDoRegister()" style="width:100%;margin-top:8px;padding:14px;border-radius:12px;border:none;background:#9333ea;color:#fff;font-size:14px;font-weight:800;cursor:pointer;">Daftar</button>
    <div style="font-size:11px;color:var(--muted);text-align:center;margin-top:14px;line-height:1.5;">Setelah daftar, akun perlu disetujui Bos dulu sebelum bisa login.</div>
  </div>
</div>
...
<div class="screen" id="screen-tipe">
  <div style="position:absolute;top:14px;right:14px;display:flex;align-items:center;gap:8px;z-index:5;">
    <div id="auth-user-badge" style="font-size:10px;color:var(--muted);font-weight:700;"></div>
    <button onclick="authLogout()" style="padding:5px 10px;border-radius:8px;border:1px solid var(--border);background:var(--surface2);color:var(--muted);font-size:10px;font-weight:700;cursor:pointer;">Keluar</button>
  </div>
  ...
  <div class="screen-body" style="padding:16px;display:flex;flex-direction:column;gap:12px;">
    ...
    <div id="tmasuk-form" style="display:none;">
      ...
      <div style="margin-bottom:14px;">
        ...
        <div style="padding:10px 12px;border-radius:8px;background:var(--surface2);border:1px solid var(--border);font-size:13px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px;">
          <span>👤</span><span id="tmasuk-operator-name">—</span>
        </div>
  ...
  <div class="screen-body" style="padding:16px;">
    ...
    <div id="tkeluar-form" style="display:none;">
      ...
      <div style="margin-bottom:14px;">
        <div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;">Operator</div>
        <div style="padding:10px 12px;border-radius:8px;background:var(--surface2);border:1px solid var(--border);font-size:13px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px;">
          <span>👤</span><span id="tkeluar-operator-name">—</span>
        </div>
      </div>
    ...
    <div id="peta-pane">
      <div class="peta-legend">
        ...
        <div style="margin-left:auto;display:flex;gap:6px;">
          <button onclick="petaToggle3D()" id="peta-3d-toggle-btn" style="padding:5px 12px;border:1px solid #9333ea40;border-radius:8px;background:#9333ea15;color:#9333ea;font-size:10px;font-weight:700;cursor:pointer;">🧊 3D</button>
      ...
      <div id="peta-3d-rack-selector" style="display:none;padding:8px 2px 12px;">
        <div style="font-size:10px;color:var(--muted);font-weight:700;text-transform:uppercase;margin-bottom:6px;">Pilih Rak</div>
        <div id="peta-3d-rack-chips" style="display:flex;flex-wrap:wrap;gap:6px;"></div>
      </div>
...
<div class="screen" id="screen-form">
  ...
  <div class="screen-body">
    ...
    <div class="form-section">
      ...
      <div id="operator-auto-display" style="padding:12px 14px;border-radius:10px;background:var(--surface2);border:1px solid var(--border);font-size:14px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:8px;">
        <span>👤</span><span id="operator-auto-name">—</span>
      </div>
    </div>
...
<div class="screen" id="screen-opname-fifo">
  ...
  <div style="padding:0 16px 100px;">
    ...
    <!-- Banner draft tersimpan — muncul otomatis kalau ada draft di sessionStorage -->
    <div id="of-draft-banner" style="display:none;margin-top:10px;padding:12px 14px;background:#fefce8;border:1.5px solid #fbbf24;border-radius:10px;justify-content:space-between;align-items:center;">
      <div>
        <div style="font-size:13px;font-weight:800;color:#92400e;">📋 Ada draft tersimpan</div>
        <div id="of-draft-info" style="font-size:11px;color:#b45309;margin-top:2px;"></div>
      </div>
      <div style="display:flex;gap:8px;">
        <button onclick="ofRestoreDraft()" style="padding:7px 12px;border-radius:8px;border:none;background:#f59e0b;color:#fff;font-size:12px;font-weight:800;cursor:pointer;">Pulihkan</button>
        <button onclick="ofDiscardDraft()" style="padding:7px 10px;border-radius:8px;border:1px solid #fbbf24;background:none;color:#92400e;font-size:12px;cursor:pointer;">Buang</button>
      </div>
    </div>
    ...
    <div style="margin-top:16px;">
      ...
      <input type="text" id="of-operator-input" placeholder="Tulis nama operator..." oninput="ofOperator = this.value.trim(); _ofScheduleDraft();" style="width:100%;margin-top:6px;padding:11px 14px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:14px;box-sizing:border-box;">
    </div>

    <div style="margin-top:14px;">
      <label style="font-size:11px;color:var(--muted);font-weight:700;text-transform:uppercase;">Tanggal Opname</label>
      <input type="date" id="of-tanggal-input" oninput="ofTanggal = this.value; _ofScheduleDraft();" style="width:100%;margin-top:6px;padding:11px 14px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:14px;box-sizing:border-box;">
    </div>

    <div style="margin-top:14px;">
      <label style="font-size:11px;color:var(--muted);font-weight:700;text-transform:uppercase;">Plant</label>
      <select id="of-plant-select" onchange="ofPlant = this.value || null; _ofScheduleDraft();" style="width:100%;margin-top:6px;padding:11px 14px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:14px;box-sizing:border-box;appearance:none;">
    ...
    <div style="margin-top:16px;display:flex;gap:8px;">
      <button onclick="ofSaveLokal()" style="flex:1;padding:12px;border-radius:10px;border:1.5px solid #2563eb;background:none;color:#2563eb;font-size:13px;font-weight:800;cursor:pointer;">💾 Save Lokal</button>
      <button onclick="ofLoadLokal()" style="flex:1;padding:12px;border-radius:10px;border:1.5px solid #2563eb;background:none;color:#2563eb;font-size:13px;font-weight:800;cursor:pointer;">📂 Load Lokal</button>
    </div>

    <button onclick="ofSave()" id="of-save-btn" style="margin-top:12px;width:100%;padding:14px;border-radius:12px;border:none;background:#9333ea;color:#fff;font-size:14px;font-weight:800;cursor:pointer;">💾 Simpan Opname</button>
  </div>
</div>
...
var PUBLIC_ACTIONS = ['registerUser', 'loginUser']; // action yang tidak butuh token
function gasCall(action, params, onSuccess, onError) {
  var url = GAS_URL + '?action=' + encodeURIComponent(action)
          + '&_t=' + Date.now(); // cache busting

  // Token dikirim TERPISAH dari params — supaya tidak merusak action yang
  // kirim params sebagai primitive (string/angka), bukan object.
  if (PUBLIC_ACTIONS.indexOf(action) === -1) {
    var authToken = _authGetToken();
    if (!authToken) {
      _authForceLogout('Sesi tidak ditemukan, silakan login.');
      if (onError) onError({ msg: 'Belum login' });
      return;
    }
  ...
  .then(function(data) {
    if (data && data.authError) {
      _authForceLogout(data.msg || 'Sesi tidak valid, silakan login ulang.');
...
var _capConfigReady = false; // true setelah getBinCapConfig dari sheet BIN_CAP selesai (sukses/gagal)

// ═══════════════════════════════════════
// INIT
// ═══════════════════════════════════════
// ═══════════════════════════════════════
// AUTH — session localStorage, gate sebelum app jalan
// ═══════════════════════════════════════
var AUTH_KEY = 'binloc_auth_session';
var _currentUser = null; // { username, nama, role, token }
...
function _authSaveSession(data) {
  try {
    localStorage.setItem(AUTH_KEY, JSON.stringify(data));
    _currentUser = data;
  } catch(e) { /* localStorage tidak tersedia — tetap jalan di memori untuk sesi ini */ _currentUser = data; }
}

function _authForceLogout(msg) {
...
function authSwitchTab(tab) {
  ...
  var daftarTab = document.getElementById('auth-tab-daftar');
  var isLogin = tab === 'login';
  loginTab.style.background  = isLogin ? 'var(--surface)' : 'none';
  loginTab.style.color       = isLogin ? 'var(--text)' : 'var(--muted)';
  daftarTab.style.background = !isLogin ? 'var(--surface)' : 'none';
  daftarTab.style.color      = !isLogin ? 'var(--text)' : 'var(--muted)';
  document.getElementById('auth-form-login').style.display  = isLogin ? 'block' : 'none';
  document.getElementById('auth-form-daftar').style.display = !isLogin ? 'block' : 'none';
  document.getElementById('auth-login-error').style.display  = 'none';
  document.getElementById('auth-daftar-error').style.display = 'none';
  document.getElementById('auth-daftar-success').style.display = 'none';
}
...
function authDoLogin() {
  ...
  if (!username || !password) {
    ...
    showToast('⚠️ Isi username dan password');
    return;
  }

  btn.disabled = true; btn.textContent = 'Memproses...';
  gasCall('loginUser', { username: username, password: password }, function(res) {
    btn.disabled = false; btn.textContent = 'Masuk';
    if (!res || !res.ok) {
      var msg = (res && res.msg) || 'Login gagal';
      errEl.textContent = msg;
      errEl.style.display = 'block';
      showToast('❌ ' + msg, 3000);
      return;
    }
    _authSaveSession({ username: res.username, nama: res.nama, role: res.role, token: res.token });
    document.getElementById('auth-login-password').value = '';
    showToast('✅ Berhasil masuk — selamat datang, ' + (res.nama || res.username));
    _startAppAfterAuth();
  }, function() {
    btn.disabled = false; btn.textContent = 'Masuk';
    errEl.textContent = 'Gagal terhubung ke server';
    errEl.style.display = 'block';
    showToast('❌ Gagal terhubung ke server', 3000);
  });
}

function authDoRegister() {
  var nama     = document.getElementById('auth-daftar-nama').value.trim();
  var username = document.getElementById('auth-daftar-username').value.trim();
  var password = document.getElementById('auth-daftar-password').value;
  var errEl = document.getElementById('auth-daftar-error');
  var okEl  = document.getElementById('auth-daftar-success');
  var btn = document.getElementById('auth-daftar-btn');
  errEl.style.display = 'none';
  ...
  btn.disabled = true; btn.textContent = 'Memproses...';
  gasCall('registerUser', { nama: nama, username: username, password: password }, function(res) {
    btn.disabled = false; btn.textContent = 'Daftar';
    if (!res || !res.ok) {
      var msg = (res && res.msg) || 'Pendaftaran gagal';
      errEl.textContent = msg;
      errEl.style.display = 'block';
      showToast('❌ ' + msg, 3000);
      return;
    }
    okEl.textContent = '✅ ' + res.msg;
    okEl.style.display = 'block';
    showToast('✅ ' + res.msg, 3500);
    window.scrollTo(0, 0);
    document.getElementById('auth-daftar-nama').value = '';
    document.getElementById('auth-daftar-username').value = '';
    document.getElementById('auth-daftar-password').value = '';
  }, function() {
    btn.disabled = false; btn.textContent = 'Daftar';
    errEl.textContent = 'Gagal terhubung ke server';
    errEl.style.display = 'block';
...
function authLogout() {
  _authForceLogout(null);
}

// Dipanggil setelah auth berhasil (baik dari session tersimpan maupun baru login)
function _startAppAfterAuth() {
  document.querySelectorAll('.screen').forEach(function(s){ s.classList.remove('active'); });
  window.scrollTo(0, 0); // reset scroll dari screen-auth sebelum tampilkan app

  var badge = document.getElementById('auth-user-badge');
  if (badge && _currentUser) badge.textContent = '👤 ' + (_currentUser.nama || _currentUser.username);
  ...
  gasCall('getBinCapConfig', {},
    function(capResult) {
      if (capResult && capResult.cfg && capResult.cfg.length) PETA_CAP_CFG = capResult.cfg;
      _capConfigReady = true;
    },
    function() { _capConfigReady = true; }
  );
}

window.addEventListener('DOMContentLoaded', function() {
  var session = _authGetSession();
...
function showScreen(id) {
  ...
  window.scrollTo(0, 0); // reset posisi scroll — cegah "nyangkut" di tengah dari screen sebelumnya
  if (id === 'screen-search') {
...
function resetState() {
  ...
  state.aksi = null;
  state.sku = null;
  // Operator otomatis dari user yang login — bukan pilihan chip bebas lagi
  state.operator = _currentUser ? (_currentUser.nama || _currentUser.username) : null;
  var opDisplay = document.getElementById('operator-auto-name');
  if (opDisplay) opDisplay.textContent = state.operator || '—';
...
// ═══════════════════════════════════════
// OPERATOR — otomatis dari user yang login (lihat resetState), tidak ada
// pilihan chip bebas lagi untuk transaksi utama.
// ═══════════════════════════════════════
...
function submitForm() {
  ...
  if (state.aksi === 'MASUK') {
    if (!_capConfigReady) {
      showToast('⏳ Menyiapkan data kapasitas, coba lagi sebentar...', 2000);
      return;
    }
...
function petaRefresh() {
  _petaLoaded = false;
  _petaData   = null;
  _peta3DGroups = null; // paksa refetch struktur LEVEL/DEPTH juga
  if (_peta3DMode) {
    gasCall('getBinCurrentLite', null, function(data) {
      _petaData = data || [];
      _petaLoaded = true;
      _peta3DLoadGroups();
    }, function() {
...
function petaLoadMap() {
  // Cap config SELALU di-fetch ulang tiap buka tab Peta (request ringan) —
  // supaya kalau device lain baru saja edit kapasitas, perubahan langsung
  // kebaca di sini tanpa perlu reload app.
  gasCall('getBinCapConfig', {}, function(capResult) {
    if (capResult && capResult.cfg && capResult.cfg.length) PETA_CAP_CFG = capResult.cfg;
    _petaAfterCapLoad();
  }, function() {
    _petaAfterCapLoad(); // gagal fetch cap → tetap lanjut pakai cap yang ada di memory
  });
}

function _petaAfterCapLoad() {
...
function petaRenderMap(rawData) {
  // Kembalikan style default (max-content) — override 3D sebelumnya (kalau ada) tidak boleh nyangkut di 2D
  var wrapEl = document.getElementById('peta-map-wrap');
  if (wrapEl) { wrapEl.style.width = ''; wrapEl.style.minWidth = ''; wrapEl.style.maxWidth = ''; wrapEl.style.margin = ''; }
...
// ═══════════════════════════════════════
// PETA 3D — isometric per-rak, kapasitas dari BIN_CAP kolom I-L (BIN/ROW/LEVEL/DEPTH)
// Urutan isi: level PALING ATAS duluan, dalam 1 level posisi PALING BELAKANG duluan.
// ═══════════════════════════════════════
var _peta3DMode      = false;
var _peta3DGroups    = null; // cache dari getBinCap3D
var _peta3DSelected  = null; // huruf bin yang lagi dipilih, misal 'A'
var _peta3DSelectedPosIdx = null; // index posisi yang di-tap (0-based), null = tidak ada

var P3D_TILE_W = 30, P3D_TILE_H = 17;
var P3D_LEVEL_H = Math.sqrt(Math.pow(P3D_TILE_W/2, 2) + Math.pow(P3D_TILE_H/2, 2)) * 1.12;

function p3dIsoProject(x, y, z) {
  return {
    x: (x - y) * (P3D_TILE_W / 2),
    y: (x + y) * (P3D_TILE_H / 2) - z * P3D_LEVEL_H
  };
}

function p3dShadeColor(hex, percent) {
...
var P3D_EKSPOR_COLOR = '#ddb96e';
var P3D_MONTH_COLOR = {
  'jan': '#ff0000', 'feb': '#ff0000',
  'mar': '#f7ff00', 'apr': '#f7ff00',
  'may': '#19e619', 'jun': '#19e619',
  'jul': '#3b82f6', 'aug': '#3b82f6',
  'sep': '#f97316', 'oct': '#f97316',
  'nov': '#efefef', 'dec': '#efefef'
};

// Warna kubus per-ITEM (bukan per-posisi lagi) — EKSPOR warna tetap, LOKAL ikut bulan prodate.
function p3dColorForItem(item) {
  var tipe = String(item.tipe || 'LOKAL').toUpperCase();
  if (tipe === 'EKSPOR') return P3D_EKSPOR_COLOR;
  var prodate = String(item.prodate || '').trim();
  var m = prodate.match(/[A-Za-z]{3}/); // ambil 3 huruf bulan dari format "DD-Mon-YYYY"
  var key = m ? m[0].toLowerCase() : '';
  return P3D_MONTH_COLOR[key] || '#f97316'; // fallback oranye kalau prodate tidak kebaca
}

function p3dColorInfo(hexOrEmpty) {
  var navy = '#1e293b';
  if (!hexOrEmpty) {
    return { color: '#bfe3f0', opacity: 0.14, strokeColor: navy, strokeOp: 0.9 };
  }
  return { color: hexOrEmpty, opacity: 1, strokeColor: navy, strokeOp: 1 };
}

// Warna & ketebalan garis tegak rangka/kotak — navy konsisten, TIDAK ada gradasi/variasi per level
// (sesuai keputusan final: semua level harus sama persis, warna maupun tebal).
var P3D_RACK_COLOR = '#1e3a8a';
var P3D_RACK_WIDTH  = 3;

function p3dCubePolygons(x, y, z, colorInfo, drawFlags) {
  var isFilled = colorInfo.opacity === 1;

  // Kotak terisi = ukuran diperkecil (ada celah antar kotak, kayak produk beneran di rak).
  // Kosong (aquarium) tetap ukuran penuh 1 sel, biar kotak kacanya jelas nempel rapat.
  var s = isFilled ? 0.94 : 1;
  var pad = (1 - s) / 2;
  var x0 = x + pad, x1 = x + 1 - pad;
  var y0 = y + pad, y1 = y + 1 - pad;
  var z0 = z + 0.06, z1 = z + 1; // atas pas rata batas rangka (tidak nongol lagi), celah cuma di bawah

  var p = {};
  p.top_fl = p3dIsoProject(x0, y0, z1);
  p.top_fr = p3dIsoProject(x1, y0, z1);
  p.top_bl = p3dIsoProject(x0, y1, z1);
  p.top_br = p3dIsoProject(x1, y1, z1);
  p.bot_fl = p3dIsoProject(x0, y0, z0);
  p.bot_fr = p3dIsoProject(x1, y0, z0);
  p.bot_bl = p3dIsoProject(x0, y1, z0);
  p.bot_br = p3dIsoProject(x1, y1, z0);
  function pts(arr) { return arr.map(function(pt){ return pt.x.toFixed(1)+','+pt.y.toFixed(1); }).join(' '); }

  var shadeTop = colorInfo.color;
  var shadeX   = p3dShadeColor(colorInfo.color, -40);
  var shadeY   = p3dShadeColor(colorInfo.color, -22);

  // Kotak terisi: stroke HITAM TIPIS di badan kotak (pemisah antar kotak).
  // Kosong (aquarium): tetap pakai garis navy biar kotak kacanya jelas per-slot.
  ...
  // Garis tegak (cuma sisi TINGGI kotak, warna navy sama kayak rangka) — HANYA untuk kotak terisi.
  // Digambar terpisah, setelah face, supaya "menang" menutupi stroke hitam tipis di posisi tegak.
  var edges = [];
  if (isFilled) {
    edges = [
      { p1: p.bot_bl, p2: p.top_bl, w: P3D_RACK_WIDTH }, // kiri
      { p1: p.bot_br, p2: p.top_br, w: P3D_RACK_WIDTH }, // tengah-depan
      { p1: p.bot_fr, p2: p.top_fr, w: P3D_RACK_WIDTH }  // kanan
    ];
    // Garis diagonal dekorasi di sisi depan (faceY) — warna sama rangka, lebih tipis dari
    // tiang (P3D_RACK_WIDTH) tapi lebih tebal dari stroke hitam pemisah kotak (0.5).
    // Level ganjil & depth ganjil → miring kiri ("/"), selain itu → miring kanan ("\").
    var levelGanjil = (z % 2) === 1;
    var depthGanjil = (y % 2) === 1;
    var miringKiri = levelGanjil && depthGanjil;
    var decorP1 = miringKiri ? p.top_br : p.top_bl;
    var decorP2 = miringKiri ? p.bot_bl : p.bot_br;
...
function petaToggle3D() {
  _peta3DMode = !_peta3DMode;
  var btn = document.getElementById('peta-3d-toggle-btn');
  var selector = document.getElementById('peta-3d-rack-selector');
  if (_peta3DMode) {
    ...
    btn.style.background = '#9333ea15';
    btn.style.color = '#9333ea';
    btn.textContent = '🧊 3D';
    selector.style.display = 'none';
    _peta3DSelectedPosIdx = null;
    petaRenderMap(_petaData || []);
  }
}

function _peta3DLoadGroups() {
  document.getElementById('peta-map-wrap').innerHTML =
    '<div style="padding:40px;text-align:center;color:var(--muted);font-size:12px">⏳ Memuat struktur rak...</div>';
  if (_peta3DGroups) { _peta3DAfterGroupsLoaded(); return; }
  gasCall('getBinCap3D', {}, function(res) {
    if (!res || !res.ok) {
      var reason = (res && res.msg) ? res.msg : 'Tidak ada respons dari server';
      document.getElementById('peta-map-wrap').innerHTML =
        '<div style="padding:40px;text-align:center;color:#ef4444;font-size:12px">❌ Gagal memuat struktur rak<br><span style="color:var(--muted);font-size:10px">' + escHtml(reason) + '</span><br>'
        + '<button onclick="_peta3DGroups=null;_peta3DLoadGroups();" style="margin-top:10px;padding:6px 14px;border:1px solid #ef444440;border-radius:8px;background:#ef444415;color:#ef4444;font-size:11px;font-weight:700;cursor:pointer;">🔄 Coba Lagi</button></div>';
      return;
    }
    _peta3DGroups = res.groups || [];
    _peta3DAfterGroupsLoaded();
  }, function(e) {
    // e bisa berupa {msg:...} dari server ATAU native JS Error (exception di kode render) — tampilkan apa adanya
    var reason = (e && e.msg) ? e.msg : (e && e.message) ? ('Bug di kode: ' + e.message) : 'Koneksi gagal / server tidak merespons';
    document.getElementById('peta-map-wrap').innerHTML =
      '<div style="padding:40px;text-align:center;color:#ef4444;font-size:12px">❌ Gagal memuat struktur rak<br><span style="color:var(--muted);font-size:10px">' + escHtml(reason) + '</span><br>'
      + '<button onclick="_peta3DGroups=null;_peta3DLoadGroups();" style="margin-top:10px;padding:6px 14px;border:1px solid #ef444440;border-radius:8px;background:#ef444415;color:#ef4444;font-size:11px;font-weight:700;cursor:pointer;">🔄 Coba Lagi</button> '
      + '<button onclick="petaDebugRaw3D()" style="margin-top:10px;padding:6px 14px;border:1px solid #64748b40;border-radius:8px;background:#64748b15;color:#64748b;font-size:11px;font-weight:700;cursor:pointer;">🔍 Lihat Respons Mentah</button></div>';
  });
}

// Debug — fetch action getBinCap3D langsung, tampilkan teks mentah apa adanya
// (tanpa parsing JSON) supaya kelihatan kalau server balikin HTML error / timeout page
function petaDebugRaw3D() {
  var authToken = _authGetToken();
  var url = GAS_URL + '?action=getBinCap3D&_t=' + Date.now() + '&token=' + encodeURIComponent(authToken || '');
  document.getElementById('peta-map-wrap').innerHTML =
    '<div style="padding:20px;font-size:11px;color:var(--muted);">⏳ Mengambil respons mentah...</div>';
  fetch(url, { redirect: 'follow', cache: 'no-store' })
  .then(function(r) { return r.text().then(function(t){ return { status: r.status, text: t }; }); })
  .then(function(res) {
    document.getElementById('peta-map-wrap').innerHTML =
      '<div style="padding:16px;font-size:11px;">'
      + '<div style="font-weight:800;margin-bottom:6px;">HTTP Status: ' + res.status + '</div>'
      + '<div style="background:#111827;color:#4ade80;padding:12px;border-radius:8px;font-family:monospace;font-size:10px;white-space:pre-wrap;word-break:break-all;max-height:300px;overflow:auto;text-align:left;">' + escHtml(res.text.substring(0, 2000)) + '</div>'
      + '<button onclick="_peta3DGroups=null;_peta3DLoadGroups();" style="margin-top:10px;padding:6px 14px;border:1px solid #ef444440;border-radius:8px;background:#ef444415;color:#ef4444;font-size:11px;font-weight:700;cursor:pointer;">🔄 Kembali</button>'
      + '</div>';
  })
  .catch(function(err) {
    document.getElementById('peta-map-wrap').innerHTML =
      '<div style="padding:20px;color:#ef4444;font-size:11px;">❌ fetch() gagal total: ' + escHtml(String(err && err.message || err)) + '</div>';
  });
}

function _peta3DAfterGroupsLoaded() {
  var chipsEl = document.getElementById('peta-3d-rack-chips');
  if (!_peta3DGroups || !_peta3DGroups.length) {
    chipsEl.innerHTML = '';
    document.getElementById('peta-map-wrap').innerHTML =
      '<div style="padding:40px;text-align:center;color:var(--muted);font-size:12px">Belum ada data LEVEL/DEPTH di BIN_CAP kolom I-L</div>';
    return;
  }
  // Dedup huruf bin — satu huruf bisa punya beberapa baris di sheet (depth beda per rentang row),
  // tapi cukup 1 chip per huruf; render-nya nanti gabungkan semua rentang punya huruf itu.
  var uniqueLetters = [];
  _peta3DGroups.forEach(function(g) { if (uniqueLetters.indexOf(g.bin) === -1) uniqueLetters.push(g.bin); });

  if (!_peta3DSelected) _peta3DSelected = uniqueLetters[0];
  chipsEl.innerHTML = uniqueLetters.map(function(letter) {
    var sel = letter === _peta3DSelected;
    return '<div onclick="petaSelect3DRack(\'' + letter + '\')" style="padding:7px 14px;border-radius:8px;cursor:pointer;font-size:12px;font-weight:800;'
      + (sel ? 'background:#9333ea;color:#fff;' : 'background:var(--surface2);color:var(--muted);border:1px solid var(--border);')
      + '">' + letter + '</div>';
  }).join('');
  petaRender3D(_peta3DSelected);
}

function petaSelect3DRack(bin) {
  _peta3DSelected = bin;
  _peta3DSelectedPosIdx = null; // reset selection posisi saat ganti rak
  _peta3DAfterGroupsLoaded();
}

function petaTap3DPos(posIdx) {
  var info = _peta3DHoverInfo[posIdx];
  if (!info) return;
  var binCode = info.binCode;

  if (_peta3DSelectedPosIdx === posIdx) {
    // Tap posisi yang sama lagi → batalkan pilihan
    _peta3DSelectedPosIdx = null;
    petaRender3D(_peta3DSelected);
    petaClosePopup();
    return;
  ...
  _peta3DSelectedPosIdx = posIdx;
  petaRender3D(_peta3DSelected);
  petaShowPopup(binCode);
}

var _peta3DHoverInfo = {}; // key posIdx -> { binCode, used, capacity, items }

function petaRender3D(binLetter) {
  var segments = (_peta3DGroups || []).filter(function(g) { return g.bin === binLetter; });
  var wrap = document.getElementById('peta-map-wrap');
  if (!segments.length) { wrap.innerHTML = '<div style="padding:40px;text-align:center;color:var(--muted);font-size:12px">Rak tidak ditemukan</div>'; return; }
  segments.sort(function(a, b) { return a.rowFrom - b.rowFrom; });

  // .peta-map-wrap defaultnya width:max-content (buat grid 2D yang perlu scroll horizontal) —
  // override di sini supaya 3D bisa full-width, terutama penting di desktop layar lebar.
  // Dikasih batas wajar (900px) + center biar tidak melebar berlebihan di monitor ultra-wide.
  wrap.style.width = '100%';
  wrap.style.minWidth = '0';
  wrap.style.maxWidth = '900px';
  wrap.style.margin = '0 auto';

  // Gabungkan semua rentang (row range) punya huruf ini — misal G: 1-25 depth4, 26-32 depth6.
  // Origin dari row terkecil, nx = span total. Tiap posisi (x) cari sendiri segmen mana yang
  // menaunginya untuk tahu depth & level KHUSUS posisi itu (bisa beda-beda per posisi).
  ...
  // Rak-rak ini row 1 fisiknya mulai dari KANAN (bukan kiri) — jadi arah xi→rowNum dibalik.
  var P3D_REVERSED_BINS = ['B','D','F','H','I'];
  var isReversed = P3D_REVERSED_BINS.indexOf(binLetter) !== -1;
  function rowNumFor(xi) {
    return isReversed ? (maxRow - xi) : (origin + xi);
  }
  ...
  var ny = maxNy; // dipakai untuk bounding box/lantai (posisi depth lebih kecil cuma isi sebagian)
  var nz = maxNz;

  // Agregasi occupancy + item per posisi bin (pakai data _petaData yang sudah ada)
  var binMap = {};
  (_petaData || []).forEach(function(r) {
    if (!binMap[r.binLoc]) binMap[r.binLoc] = { used: 0, tipeCounts: {}, items: [] };
    var pecList = (r.pecahan_list && r.pecahan_list.length > 0) ? r.pecahan_list : ((r.pecahan||0) > 0 ? [r.pecahan] : []);
    binMap[r.binLoc].used += (r.pallet || 0) + pecList.length;
    var tp = String(r.tipe || 'LOKAL').toUpperCase();
    binMap[r.binLoc].tipeCounts[tp] = (binMap[r.binLoc].tipeCounts[tp] || 0) + 1;
    binMap[r.binLoc].items.push(r);
  });

  // Hitung filled-set per posisi: level PALING ATAS duluan, lalu depth PALING BELAKANG duluan.
  // Warna sekarang per-SLOT (bukan per-posisi) — tiap item beda warna sesuai tipe/prodate-nya.
  var filledSet  = {}; // key 'x_y_z' -> true
  var slotColor  = {}; // key 'x_y_z' -> warna hex khusus slot itu
  _peta3DHoverInfo = {};
  for (var xi = 0; xi < nx; xi++) {
    ...
    var capacity = ny_local * nz_local;
    var used = info ? info.used : 0;

    _peta3DHoverInfo[xi] = { binCode: binCode, rowNum: rowNum, used: used, capacity: capacity, items: info ? info.items : [] };
    ...
    // Isi slot berurutan per ITEM — tiap item "mengklaim" beberapa slot berturut-turut
    // sesuai jumlah plt efektifnya, dengan warna item itu sendiri.
    var slotPtr = 0;
    if (info && info.items && info.items.length) {
      info.items.forEach(function(item) {
        var pecList = (item.pecahan_list && item.pecahan_list.length > 0) ? item.pecahan_list : ((item.pecahan||0) > 0 ? [item.pecahan] : []);
        var unitsForItem = (item.pallet || 0) + pecList.length;
        var itemColor = p3dColorForItem(item);
        for (var u = 0; u < unitsForItem && slotPtr < order.length; u++, slotPtr++) {
          var yz = order[slotPtr][0] + '_' + order[slotPtr][1];
          filledSet[xi + '_' + yz] = true;
          slotColor[xi + '_' + yz] = itemColor;
        }
      });
    }
  }
  ...
  for (var z2 = 0; z2 < nz; z2++) {
    for (var y2 = 0; y2 < ny; y2++) {
      for (var x2 = 0; x2 < nx; x2++) {
        if (!existsAt(x2, y2, z2)) continue; // di luar depth/level posisi ini — tidak ada kubus di sana sama sekali
        var selfFilled = isFilledAt(x2, y2, z2);
        if (!selfFilled) {
          // Kosong = tidak digambar (transparan), TAPI tetap sisipkan area klik kecil
          // di urutan gambar yang SAMA (depth-sort bareng kubus asli) — supaya klik/hover
          // tidak "nyerempet" ke kolom lain akibat proyeksi iso yang melebar.
          cubes.push({ x:x2, y:y2, z:z2, invisible:true, binIdx:x2 });
          continue;
        }
        // Kotak terisi = ada celah antar kotak (gaya "produk terpisah"), jadi sisi kanan/depan
        // SELALU digambar (tidak pernah ketutup tetangga). Cuma sisi ATAS yang dicek —
        // kalau ada kubus terisi tepat di atasnya, atasnya tidak usah digambar (tersembunyi).
        var drawTop   = !isFilledAt(x2, y2, z2+1);
        var drawFaceX = true;
        var drawFaceY = true;
        var cInfo = p3dColorInfo(slotColor[x2+'_'+y2+'_'+z2]);
        // Kalau ada posisi yang dipilih, redupkan semua posisi LAIN
        if (_peta3DSelectedPosIdx !== null && x2 !== _peta3DSelectedPosIdx) {
          cInfo = { color: cInfo.color, opacity: cInfo.opacity * 0.16, strokeColor: cInfo.strokeColor, strokeOp: cInfo.strokeOp * 0.35 };
        }
        cubes.push({ x:x2, y:y2, z:z2, colorInfo: cInfo, drawTop:drawTop, drawFaceX:drawFaceX, drawFaceY:drawFaceY, binIdx:x2 });
      }
    }
  }
  cubes.sort(function(a, b) { return (a.x + a.y - a.z) - (b.x + b.y - b.z); });

  // Bounding box mencakup SELURUH rangka (0..nx, 0..ny, 0..nz) — bukan cuma kubus terisi,
  // karena sekarang posisi kosong tidak lagi punya entri kubus sama sekali.
  var minX=1e9,minY=1e9,maxX=-1e9,maxY=-1e9;
  for (var bbx=0; bbx<=nx; bbx++) for (var bby=0; bby<=ny; bby++) for (var bbz=0; bbz<=nz; bbz++) {
    var p = p3dIsoProject(bbx, bby, bbz);
    if (p.x < minX) minX = p.x; if (p.x > maxX) maxX = p.x;
    if (p.y < minY) minY = p.y; if (p.y > maxY) maxY = p.y;
  }
  var labelSpace = 26; // ruang tambahan di bawah untuk nomor posisi
  var pad = 30;
  ...
  // ── Lantai solid — supaya rak tidak kelihatan mengambang, terutama yang kosong ──
  var floorPad = 0.15; // sedikit lebih lebar dari footprint asli, biar kelihatan jelas kayak alas
  var floorPts = [
    p3dIsoProject(-floorPad, -floorPad, 0), p3dIsoProject(nx+floorPad, -floorPad, 0),
    p3dIsoProject(nx+floorPad, ny+floorPad, 0), p3dIsoProject(-floorPad, ny+floorPad, 0)
  ...
  // ── Rangka rak — tiang PER-LEVEL + zigzag per level. SEKARANG digabung ke urutan
  // depth-sort YANG SAMA dengan kotak (bukan "semua rangka duluan"), jadi tiap elemen
  // rangka bisa di-depan/di-belakang kotak tertentu sesuai posisi 3D aslinya — seperti
  // "bring forward/send backward" di PowerPoint, otomatis konsisten secara 3D.
  ...
  var renderList = cubes.slice(); // salinan — kubus (termasuk yang invisible) + elemen rangka digabung di sini
  ...
  // Kubus terisi diberi prioritas +0.9 (bukan 0.4) — supaya menang dibanding rangka di
  // KEDUA batas kolomnya sendiri (kiri DAN kanan), tidak cuma salah satu sisi. Sebelumnya
  // +0.4 cuma cukup menang di batas kanan, batas kiri masih kalah dan zigzag nembus kotak.
  renderList.forEach(function(item) {
    if (item.frameType) return; // sudah ada depth
    item.depth = (item.x + item.y - item.z) + (item.invisible ? 0 : 0.9);
  });
  renderList.sort(function(a, b) { return a.depth - b.depth; });

  renderList.forEach(function(item) {
    if (item.frameType === 'post') {
      var fpBot = p3dIsoProject(item.fx, item.fy, item.fzL), fpTop = p3dIsoProject(item.fx, item.fy, item.fzL+1);
      svgParts.push('<line x1="'+fpBot.x.toFixed(1)+'" y1="'+fpBot.y.toFixed(1)+'" x2="'+fpTop.x.toFixed(1)+'" y2="'+fpTop.y.toFixed(1)+'" stroke="'+P3D_RACK_COLOR+'" stroke-width="'+P3D_RACK_WIDTH+'"/>');
      return;
    }
    if (item.frameType === 'zigzag') {
      var fflip = item.fzz % 2 === 0;
      var ffrom = fflip ? p3dIsoProject(item.fx, item.fy2, item.fzz)   : p3dIsoProject(item.fx, item.fy2, item.fzz+1);
      var fto   = fflip ? p3dIsoProject(item.fx, item.fy2+1, item.fzz+1) : p3dIsoProject(item.fx, item.fy2+1, item.fzz);
      svgParts.push('<line x1="'+ffrom.x.toFixed(1)+'" y1="'+ffrom.y.toFixed(1)+'" x2="'+fto.x.toFixed(1)+'" y2="'+fto.y.toFixed(1)+'" stroke="'+P3D_RACK_COLOR+'" stroke-width="'+P3D_RACK_WIDTH+'"/>');
      return;
    }
    var c = item;
    if (c.invisible) {
      // Sel kosong — kotak transparan kecil (1 unit), diurutkan bareng kubus asli via
      // depth-sort yang sama, jadi tidak akan pernah nyerempet nutupin kolom lain.
      var x0i=c.x, x1i=c.x+1, y0i=c.y, y1i=c.y+1, z0i=c.z, z1i=c.z+1;
      var ipts = [
        p3dIsoProject(x0i,y0i,z1i), p3dIsoProject(x1i,y0i,z1i),
        p3dIsoProject(x1i,y1i,z1i), p3dIsoProject(x1i,y1i,z0i),
        p3dIsoProject(x0i,y1i,z0i), p3dIsoProject(x0i,y0i,z0i)
      ].map(function(p){ return p.x.toFixed(1)+','+p.y.toFixed(1); }).join(' ');
      svgParts.push('<polygon points="'+ipts+'" fill="transparent" style="cursor:pointer;" '
        + 'onclick="petaTap3DPos('+c.binIdx+')" '
        + 'onmouseenter="petaHover3D(event,'+c.binIdx+')" onmouseleave="petaHoverOut3D()"'
      + '/>');
      return;
    }
    var result = p3dCubePolygons(c.x, c.y, c.z, c.colorInfo, { top:c.drawTop, faceX:c.drawFaceX, faceY:c.drawFaceY });
    result.faces.forEach(function(f) {
      svgParts.push('<polygon points="'+f.pts+'" fill="'+f.fill+'" fill-opacity="'+f.op+'" stroke="'+f.stroke+'" stroke-opacity="'+f.strokeOp+'" stroke-width="'+f.strokeW+'" style="cursor:pointer;" '
        + 'onclick="petaTap3DPos('+c.binIdx+')" '
        + 'onmouseenter="petaHover3D(event,'+c.binIdx+')" onmouseleave="petaHoverOut3D()"'
      + '/>');
    });
    result.edges.forEach(function(e) {
      svgParts.push('<line x1="'+e.p1.x.toFixed(1)+'" y1="'+e.p1.y.toFixed(1)+'" x2="'+e.p2.x.toFixed(1)+'" y2="'+e.p2.y.toFixed(1)+'" stroke="'+P3D_RACK_COLOR+'" stroke-width="'+e.w+'" style="pointer-events:none;"/>');
    });
  });

  // ── Nomor posisi — di garis depan-bawah tiap kolom, biar kelihatan row berapa ──
  for (var lx = 0; lx < nx; lx++) {
    var lp = p3dIsoProject(lx + 0.5, ny, 0); // titik depan-bawah kolom ini
  ...
  var infoBar = '<div style="padding:6px 4px 10px;font-size:10px;color:var(--muted);">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px;margin-bottom:6px;">'
      + '<span>Rak <b style="color:var(--text)">' + binLetter + '</b> · '
      + nx + ' posisi · ' + nz + ' level × ' + ny + ' depth</span>'
      + '<span><span style="display:inline-block;width:9px;height:9px;background:'+P3D_EKSPOR_COLOR+';border-radius:2px;margin-right:3px;"></span>Ekspor</span>'
    + '</div>'
    + '<div style="display:flex;flex-wrap:wrap;gap:8px;">'
      + '<span style="font-weight:700;">Lokal (prodate):</span>'
      + '<span><span style="display:inline-block;width:9px;height:9px;background:#ff0000;border-radius:2px;margin-right:3px;"></span>Jan-Feb</span>'
      + '<span><span style="display:inline-block;width:9px;height:9px;background:#f7ff00;border:1px solid #d4d400;border-radius:2px;margin-right:3px;"></span>Mar-Apr</span>'
      + '<span><span style="display:inline-block;width:9px;height:9px;background:#19e619;border-radius:2px;margin-right:3px;"></span>Mei-Jun</span>'
      + '<span><span style="display:inline-block;width:9px;height:9px;background:#3b82f6;border-radius:2px;margin-right:3px;"></span>Jul-Agu</span>'
      + '<span><span style="display:inline-block;width:9px;height:9px;background:#f97316;border-radius:2px;margin-right:3px;"></span>Sep-Okt</span>'
      + '<span><span style="display:inline-block;width:9px;height:9px;background:#efefef;border:1px solid #9ca3af;border-radius:2px;margin-right:3px;"></span>Nov-Des</span>'
    + '</div>'
  + '</div>';
...
function petaHover3D(evt, posIdx) {
  var info = _peta3DHoverInfo[posIdx];
  if (!info) return;
  var tip = document.getElementById('peta-3d-tooltip');
  if (!tip) return;

  var itemsLines = '';
  if (info.items && info.items.length) {
    itemsLines = info.items.slice(0, 4).map(function(r) {
      return '<div style="display:flex;justify-content:space-between;gap:8px;"><span>' + escHtml(r.skuNama || r.skuKode || '') + '</span><span style="font-weight:800;white-space:nowrap;">' + pecEff(r) + ' plt</span></div>';
    }).join('');
    if (info.items.length > 4) itemsLines += '<div style="color:var(--muted);font-size:10px;">+' + (info.items.length-4) + ' lainnya…</div>';
  } else {
    itemsLines = '<div style="color:var(--muted);">Kosong</div>';
  }

  tip.innerHTML = '<div style="font-weight:800;margin-bottom:4px;">' + escHtml(info.binCode) + ' · ' + info.used + '/' + info.capacity + ' plt</div>' + itemsLines;

  var evX = (evt.touches && evt.touches[0]) ? evt.touches[0].clientX : evt.clientX;
  var evY = (evt.touches && evt.touches[0]) ? evt.touches[0].clientY : evt.clientY;
  tip.style.left = Math.min(evX + 12, window.innerWidth - 190) + 'px';
  tip.style.top  = Math.max(evY - 10, 10) + 'px';
  tip.style.display = 'block';
}

function petaHoverOut3D() {
  var tip = document.getElementById('peta-3d-tooltip');
...
function petaClosePopup() {
  ...
  if (_peta3DMode && _peta3DSelectedPosIdx !== null) {
    _peta3DSelectedPosIdx = null;
    petaRender3D(_peta3DSelected);
  }
}
...
var ofTanggal      = null;
var ofPlant        = null;
var _ofInitializing = false; // true selama openOpnameFifo mereset state — cegah auto-save overwrite draft asli
var _ofDraftTimer   = null;
var OF_DRAFT_KEY    = 'binloc_opname_draft';
...
function ofSaveDraft() {
  if (_ofInitializing) return; // jangan simpan draft kosong saat baru reset/buka screen
  try {
    var draft = {
      rows: ofRows,
      operator: ofOperator,
    ...
    sessionStorage.setItem(OF_DRAFT_KEY, JSON.stringify(draft));
  } catch(e) { /* sessionStorage penuh/tidak tersedia, skip */ }
}

function _ofScheduleDraft() {
  clearTimeout(_ofDraftTimer);
  _ofDraftTimer = setTimeout(ofSaveDraft, 1000);
}

function ofCheckDraft() {
  try {
    var raw = sessionStorage.getItem(OF_DRAFT_KEY);
    if (!raw) { _ofHideDraftBanner(); return; }
    var draft = JSON.parse(raw);
    if (!draft || !draft.rows || !draft.rows.length) { _ofHideDraftBanner(); return; }
    var savedAt = draft.savedAt ? new Date(draft.savedAt).toLocaleTimeString('id-ID', {hour:'2-digit',minute:'2-digit'}) : '—';
    document.getElementById('of-draft-info').textContent =
      draft.rows.length + ' baris · ' + (draft.operator||'—') + ' · Plant ' + (draft.plant||'—') + ' · Disimpan ' + savedAt;
    document.getElementById('of-draft-banner').style.display = 'flex';
  } catch(e) { _ofHideDraftBanner(); }
}

function _ofHideDraftBanner() {
  var b = document.getElementById('of-draft-banner');
  if (b) b.style.display = 'none';
}

function ofRestoreDraft() {
  try {
    var raw = sessionStorage.getItem(OF_DRAFT_KEY);
    if (!raw) return;
    var draft = JSON.parse(raw);
    ofRows     = draft.rows || [];
    ofOperator = draft.operator || null;
    ofTanggal  = draft.tanggal  || _todayISODate();
    ofPlant    = draft.plant    || null;
    document.getElementById('of-operator-input').value = ofOperator || '';
    document.getElementById('of-tanggal-input').value  = ofTanggal;
    document.getElementById('of-plant-select').value   = ofPlant   || '';
    ofRenderRows();
    _ofHideDraftBanner();
    showToast('✅ Draft dipulihkan — ' + ofRows.length + ' baris');
  } catch(e) {
    showToast('⚠️ Gagal memulihkan draft');
  }
}

function ofDiscardDraft() {
  sessionStorage.removeItem(OF_DRAFT_KEY);
  _ofHideDraftBanner();
  showToast('🗑 Draft dibuang');
}

function ofClearDraft() {
  sessionStorage.removeItem(OF_DRAFT_KEY);
  _ofHideDraftBanner();
}

// ── SAVE/LOAD LOKAL — manual, key terpisah dari auto-draft supaya tidak saling timpa ──
var OF_LOKAL_KEY = 'binloc_opname_lokal';

function ofSaveLokal() {
  try {
    var data = {
      rows: ofRows,
      operator: ofOperator,
    ...
    sessionStorage.setItem(OF_LOKAL_KEY, JSON.stringify(data));
    showToast('✅ Tersimpan lokal — ' + ofRows.length + ' baris. Aman kalau refresh/gagal simpan.');
  } catch(e) {
    showToast('⚠️ Gagal simpan lokal — storage penuh/tidak tersedia');
  }
}

function ofLoadLokal() {
  try {
    var raw = sessionStorage.getItem(OF_LOKAL_KEY);
    if (!raw) { showToast('📭 Belum ada data lokal tersimpan'); return; }
    var data = JSON.parse(raw);
    if (!data || !data.rows || !data.rows.length) { showToast('📭 Belum ada data lokal tersimpan'); return; }
    _ofInitializing = true;
    ofRows     = data.rows;
    ofOperator = data.operator || null;
    ofTanggal  = data.tanggal  || _todayISODate();
    ofPlant    = data.plant    || null;
    document.getElementById('of-operator-input').value = ofOperator || '';
    document.getElementById('of-tanggal-input').value  = ofTanggal;
    document.getElementById('of-plant-select').value   = ofPlant   || '';
    ofRenderRows();
    _ofInitializing = false;
    var savedAt = data.savedAt ? new Date(data.savedAt).toLocaleTimeString('id-ID', {hour:'2-digit',minute:'2-digit'}) : '—';
    showToast('✅ Dimuat dari lokal — ' + ofRows.length + ' baris (disimpan ' + savedAt + ')');
  } catch(e) {
    showToast('⚠️ Gagal memuat data lokal');
  }
}
...
function openOpnameFifo() {
  ...
  document.getElementById('of-tanggal-input').value  = ofTanggal;
  document.getElementById('of-plant-select').value   = '';
  ofAddRow();
  showScreen('screen-opname-fifo');
  _ofInitializing = false;
  // Cek draft setelah reset selesai — banner muncul kalau ada draft asli tersimpan
  setTimeout(ofCheckDraft, 100);
}
...
function ofSave() {
  ...
  if (!ofPlant)    { showToast('⚠️ Pilih Plant dulu (1111/1112/1113)'); return; }
  ...
  gasCall('saveOpnameFifo', { rows: payload, operator: ofOperator, tanggal: ofTanggal, plant: ofPlant }, function(res) {
    ...
    document.getElementById('of-save-btn').disabled = false;
    document.getElementById('of-save-btn').textContent = '💾 Simpan Opname';
    showToast('❌ Gagal menyimpan — data draft masih tersimpan, jangan tutup app');
  });
}
...
function ofRenderRiwayat(sessions) {
  ...
  el.innerHTML = sessions.map(function(s, idx) {
    ...
    var plantBadge = s.plant ? '<span style="font-size:10px;font-weight:800;background:#1a237e15;color:#1a237e;padding:1px 7px;border-radius:4px;margin-left:6px;">Plant ' + escHtml(s.plant) + '</span>' : '';
    return '<div onclick="ofShowDetail(\'' + escHtml(s.sessionId) + '\')" style="background:var(--surface2);border-radius:10px;padding:12px 14px;margin-bottom:8px;cursor:pointer;">'
      + '<div style="display:flex;justify-content:space-between;align-items:center;">'
        + '<div>'
          + '<div style="font-size:13px;font-weight:800;color:var(--text);">' + escHtml(tglLabel) + plantBadge + '</div>'
...
function ofShowDetail(sessionId) {
  ...
  document.getElementById('of-detail-sub').textContent = s.ts + ' · Plant ' + (s.plant||'—') + ' · ' + s.rows.length + ' item · 👤 ' + s.operator;
...
function ofDownloadPdf() {
  ...
  var plant   = s.plant || '';
  var plantLabel = plant ? 'FIFO ' + plant : 'FIFO';

  var totalProdate = 0, totalPallet = 0, totalKarton = 0;
  var prodateSet = {};
  s.rows.forEach(function(r) {
    if (r.prodate && !prodateSet[r.prodate]) { prodateSet[r.prodate] = true; totalProdate++; }
    totalPallet  += Number(r.pltSum) || 0;
    totalKarton  += Number(r.total)  || 0;
  });

  var groups = [];
  var groupMap = {};
  s.rows.forEach(function(r) {
    var key = (r.kode||'') + '|' + (r.nama||'');
    if (!groupMap[key]) {
      groupMap[key] = { kode: r.kode, nama: r.nama, rows: [] };
  ...
  groups.forEach(function(g) {
    ...
    g.rows.forEach(function(r, ri) {
      var c = r.ket === 'OK' ? '#16a34a' : (Number(r.selisih) > 0 ? '#2563eb' : '#dc2626');
      var pltDisplay = Number(r.pltSum) || 0;
      var pecRaw = String(r.pecStr || '').trim();
      var pecDisplay = pecRaw ? pecRaw.split(',').map(function(x){ return x.trim(); }).filter(Boolean).join('+') : '0';
      tableRows += '<tr>';
      if (ri === 0) {
        tableRows += '<td rowspan="' + span + '" style="text-align:center;font-weight:bold;border-right:1px solid #e5e7eb;vertical-align:middle;">' + skuNo + '</td>';
        tableRows += '<td rowspan="' + span + '" style="font-family:monospace;font-size:10px;border-right:1px solid #e5e7eb;vertical-align:middle;">' + escHtml(String(g.kode||'')) + '</td>';
        tableRows += '<td rowspan="' + span + '" style="font-weight:700;border-right:1px solid #e5e7eb;vertical-align:middle;">' + escHtml(String(g.nama||'')) + '</td>';
      }
      tableRows += '<td style="text-align:center;">' + escHtml(String(r.prodate||'—')) + '</td>';
      tableRows += '<td style="text-align:center;font-weight:bold;">' + pltDisplay + '</td>';
      tableRows += '<td style="text-align:center;">' + pecDisplay + '</td>';
      tableRows += '<td style="text-align:center;font-weight:bold;">' + (Number(r.total)||0) + '</td>';
      tableRows += '<td>' + escHtml(String(r.binLoc||'—')) + '</td>';
      tableRows += '<td style="text-align:center;font-weight:bold;color:' + c + ';">' + escHtml(String(r.ket||'—')) + '</td>';
      tableRows += '</tr>';
    });
  });

  var html = '<!DOCTYPE html><html><head><meta charset="utf-8">'
    + '<title>Picking List FIFO ' + tgl + '</title>'
    + '<style>'
      + 'body{font-family:Arial,sans-serif;font-size:11px;margin:24px;color:#111;}'
      + 'h2{margin:0 0 2px;font-size:16px;font-weight:900;letter-spacing:.5px;}'
      + '.summary-boxes{display:flex;gap:12px;margin:14px 0 18px;}'
      + '.summary-box{border:1.5px solid #e5e7eb;border-radius:8px;padding:10px 18px;min-width:90px;text-align:center;}'
      + '.summary-box .val{font-size:22px;font-weight:900;color:#111;line-height:1.1;}'
      + '.summary-box .lbl{font-size:9px;color:#6b7280;font-weight:700;text-transform:uppercase;margin-top:2px;}'
      + 'table{width:100%;border-collapse:collapse;}'
      + 'th{background:#1a237e;color:#fff;padding:7px 8px;text-align:left;font-size:10px;font-weight:700;}'
      + 'td{padding:5px 8px;border-bottom:1px solid #e5e7eb;vertical-align:middle;}'
      + 'tr:nth-child(even) td{background:#f9fafb;}'
      + '.plant-badge{display:inline-block;background:#1a237e15;color:#1a237e;font-weight:800;font-size:11px;padding:2px 10px;border-radius:5px;margin-left:8px;}'
      + '@media print{button{display:none!important;}}'
    + '</style></head><body>'
    + '<h2>Picking List FIFO — ' + escHtml(tgl) + ' — ' + escHtml(s.operator) + ' <span class="plant-badge">' + escHtml(plantLabel) + '</span></h2>'
    + '<button onclick="window.print()" style="margin-bottom:4px;padding:6px 14px;background:#9333ea;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:11px;margin-top:10px;">🖨️ Print / Save PDF</button>'
    + '<div class="summary-boxes">'
      + '<div class="summary-box"><div class="val">' + totalProdate + '</div><div class="lbl">Prodate</div></div>'
      + '<div class="summary-box"><div class="val">' + totalPallet  + '</div><div class="lbl">Pallet</div></div>'
      + '<div class="summary-box"><div class="val">' + totalKarton.toLocaleString('id-ID') + '</div><div class="lbl">Total Karton</div></div>'
    + '</div>'
    + '<table><thead><tr>'
      + '<th style="width:28px;text-align:center;">#</th>'
      + '<th style="width:70px;">SKU</th>'
      + '<th style="width:180px;">Nama Barang</th>'
      + '<th style="width:90px;text-align:center;">Prodate</th>'
      + '<th style="width:50px;text-align:center;">Pallet</th>'
      + '<th style="width:55px;text-align:center;">Pecahan</th>'
      + '<th style="width:75px;text-align:center;">Total Karton</th>'
      + '<th style="width:80px;">Bin Loc</th>'
      + '<th style="width:70px;text-align:center;">Keterangan</th>'
    + '</tr></thead><tbody>' + tableRows + '</tbody></table>'
    + '<p style="margin-top:14px;color:#9ca3af;font-size:10px;">Dicetak: ' + new Date().toLocaleString('id-ID') + '</p>'
    + '</body></html>';

  var a = document.createElement('a');
  var blob = new Blob([html], { type: 'text/html' });
  a.href    = URL.createObjectURL(blob);
  a.download = 'picking-list-' + plant + '-' + tgl + '.html';
...
function openTrialMasuk(skuKode, prodateVal, quotationVal) {
  ...
  // Operator otomatis dari user yang login
  _autoFillTrialMOp();
...
// Operator otomatis dari user yang login — dipanggil saat screen trial masuk/keluar dibuka
function _autoFillTrialMOp() {
  var nama = _currentUser ? (_currentUser.nama || _currentUser.username) : '';
  document.getElementById('tmasuk-operator').value = nama;
  var el = document.getElementById('tmasuk-operator-name');
  if (el) el.textContent = nama || '—';
}

function _autoFillTrialKOp() {
  var nama = _currentUser ? (_currentUser.nama || _currentUser.username) : '';
  _trialK.operator = nama;
  var el = document.getElementById('tkeluar-operator-name');
  if (el) el.textContent = nama || '—';
}
...
function opnameSimpanSelisih(kode, nama, idx, prodateAsli, qtAsli, sysTipe, sysPlt, sysPec, sysKtn) {
  ...
  var pecInputEmpty  = !pecUserTouched;
